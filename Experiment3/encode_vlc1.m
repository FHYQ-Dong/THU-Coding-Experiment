function [num_1, code_1] = encode_vlc1(onesymbol_coodbook_file, slice_heightOption, srcImage, procImage, slice_start_code)
%vlc encode by one symbol
        ...
        bin_file = fopen('vlc1_bin.txt', 'wb');
        if ~isempty(bin_file)
            ...
            m = 1;  %index of bit
            bin_file = fopen('vlc1_bin.txt', 'r'); %read only
            if ~isempty(bin_file)
                data = fgetl(bin_file);   
                while m<=length(data)
                    if length(data)-m < 23
