function [num_2_1, num_2_2, code_2] = encode_vlc2(twosymbol_coodbook_file, slice_heightOption, srcImage, procImage, slice_start_code)
...
bin_file = fopen('vlc2_bin.txt', 'wb');
if ~isempty(bin_file)
...
%                 fprintf("x:%d\ta:%d\n",x,find(num_2_1==pix1));
    ...
    m = 1;   %index of bit
    bin_file = fopen('vlc2_bin.txt', 'r');  %read only
    if ~isempty(bin_file)
        data = fgetl(bin_file);
        while m<=length(data)
            if length(data)-m < 23
